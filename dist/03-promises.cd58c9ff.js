...
function handleSubmit(event) {
    ...
    const delay1 = parseInt(formData.get("delay"), 10);
    const step = parseInt(formData.get("step"), 10);
    const amount = parseInt(formData.get("amount"), 10);
    for(let i = 1; i <= amount; i += 1)createPromise(i, delay1 + (i - 1) * step).then(({ position , delay  })=>{
        (0, _notiflixDefault.default).Notify.success(`Fulfilled promise ${position} in ${delay}ms`);
    }).catch(({ position , delay  })=>{
        (0, _notiflixDefault.default).Notify.failure(`Rejected promise ${position} in ${delay}`);
    });
}
